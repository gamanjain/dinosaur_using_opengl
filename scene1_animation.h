...
void scene1_dino_idle() {
	...
	if (dino_movey == 1) dino_slidey -= 0.2;//1
	if (dino_slidey < -20) dino_movey = 0;
	if (dino_movey == 0) dino_slidey += 0.2;//1
	...
	if (dino_movex == 0) dino_slidex += 0.2;//4
...
void scene1_dinoLleg_animation() {
	glPushMatrix();
	if (dinoleg_count == 0) {
		glTranslatef(dino_slidex - 60, dino_slidey + 5, 0);
